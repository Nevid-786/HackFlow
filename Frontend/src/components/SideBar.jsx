  ...
  return (
    <>
      <button className='fixed top-4 left-4 z-50 md:hidden p-2 bg-primary rounded-md' onClick={() => setIsSideBar(!isSideBar)}>
        {isSideBar ? <X className='h-3 w-3' /> : <MenuSquare className='h-3 w-3' />}
      </button>
      <div className={`flex flex-col fixed md:relative  md:w-[20%] border-[#C7C4D7] justify-between h-screen px-2 py-5 bg-[#EFF4FF] z-40 transition-all duration-300 ${isSideBar ? "left-0" : "-left-full"} md:left-0`}>
        ...
        <div className="flex flex-col border-t-2 font-jetbrains ">
            <ul className='py-2 space-y-2'>
                ...
                <li className={` rounded-md flex cursor-pointer gap-x-4 p-2 ${location.pathname=="/members"?"bg-primary":""}`}  onClick={()=>{navigate("/members") ;setSelected("Members")}}><span className='h-4 w-4'><LucideCirclePoundSterling/></span>Members</li>
            </ul>
        </div>
        </div>
        <div className=" flex flex-col border-t-2 font-jetbrains ">
